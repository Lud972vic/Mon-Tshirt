...
//Recuperation de la valaeur de l'option selectionnée par défaut au chargement de la page
    let type_id = $('.change_type').val();
//Appel de la fonction et prends le select par defaut au lancement
...
$('.change_type').on('change', function () {
    ...
    //relance le fonction, si on change de select
    loadTailles(type_id, produit_id);
});
...
$('.remove_size').on('click', function (e) { //this = aliase de .remove_size
    e.preventDefault();
